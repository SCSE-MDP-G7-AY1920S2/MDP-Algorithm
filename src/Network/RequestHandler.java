...
//        System.out.println(realMap.getCell(8, 7));
    ...
    public void sendStartMsg() {
        ...
        JSONObject startPoint = new JSONObject()
                ...
                .put("x", robot.getCurLocation().x + 1)
                .put("y", robot.getCurLocation().y + 1);
...
//            System.out.println( "Connection closed" );
    ...
    private void execute_command(String cmd) throws InterruptedException {
        ...
        switch (firstChar) {
            case 'K':
                ...
                break;
            case 'W':
                robot.move(RoboCmd.FORWARD, step, exploredMap, RobotConstants.STEP_PER_SECOND);
                sendSensorRes();
                break;
            case 'S':
                robot.move(RoboCmd.BACKWARD, step, exploredMap, RobotConstants.STEP_PER_SECOND);
                sendSensorRes();
                break;
            case 'D':
    ...
    public void sendSensorRes() {
        robot.updateSensorResult(realMap);
        send(formatSensorRes(robot.getSensorResult()));
    }
