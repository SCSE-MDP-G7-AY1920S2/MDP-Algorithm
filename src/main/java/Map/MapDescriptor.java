...
//        }
    ...
    public String generateMDFString1(Map map) {
        ...
        for (int r = 0; r < MapConstants.MAP_LENGTH; r++) {
            for (int c = 0; c < MapConstants.MAP_WIDTH; c++) {
                temp.append(map.getGrid(r, c).getExplored() ? '1':'0');
    ...
    public String generateMDFString2(Map map) {
        ...
        for (int r = 0; r < MapConstants.MAP_LENGTH; r++) {
            for (int c = 0; c < MapConstants.MAP_WIDTH; c++) {
                if (map.getGrid(r, c).getExplored()) {
                    temp.append(map.getGrid(r, c).isObstacles() ? '1' : '0');
                    if (temp.length() == 4) {
        ...
        for (int r = 0; r < MapConstants.MAP_LENGTH; r++) {
            for (int c = 0; c < MapConstants.MAP_WIDTH; c++) {
                if (expStr.charAt(index) == '1') {
                    map.getGrid(r, c).setExplored(true);
                }
                index++;
            }
        }
    ...
    public void loadMDFString2(String MDFstr2, Map map) {
        ...
        for (int r = 0; r < MapConstants.MAP_LENGTH; r++) {
            for (int c = 0; c < MapConstants.MAP_WIDTH; c++) {
                MapGrid grid = map.getGrid(r, c);
                if (grid.getExplored()) {
                    if (obsStr.charAt(index) == '1') {
                        grid.setObstacles(true);
                        // create virtual wall
                        map.setVirtualWall(grid, true);
                    }
                    index++;
                }
            }
