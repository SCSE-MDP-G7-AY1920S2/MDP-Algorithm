...
public class FileManager {
    ...
    public void jsonToImage (String msg, HashMap<Point, String> imageList) {
...
//            JSONArray arr = new JSONArray(msg);
                System.out.println("byte image = " + msg);
                JSONObject imgRawJson = new JSONObject(new JSONTokener(msg));
                JSONObject recognizedImage = imgRawJson.getJSONObject(NetworkConstants.IMAGE_KEY);
                recognizedImage.keys().forEachRemaining(key -> {
                    String image = recognizedImage.getString(key);
                    if (imageList.containsValue(key)) {
                        try {
                            System.out.println("iamge list = " + imageList);
                            LOGGER.info("Saving image...");
                            String tmp = image.split("'")[1];
                            byte[] decodedString = Base64.getDecoder().decode(tmp.getBytes(StandardCharsets.UTF_8));
                            File image_f = new File(f, key + ".jpg");
                        ...
                        System.out.println("image does not exist " + key);
                    }
                });
            }
        } catch (Exception e){
